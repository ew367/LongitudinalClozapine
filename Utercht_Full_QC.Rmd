...
params: 
  Name: Utrecht
  SampleSheet: Utrecht_SampleSheet_with_pheno1.csv
  PathToProjectFolder:
  Mset: Utrecht_Mset.rdat 
  RGset: Utrecht_RGset.rdat
  IntensityThreshold: 750
  Genotype: Genodata/Utrecht_Geno_SNPs.raw
  fam: Utrecht_Geno_SNPs.fam
...
if(sum(!is.na(SampleSheet$Phenotype)) > length(unique(SampleSheet$Plate))){
...
if(sum(!is.na(SampleSheet$Time_Point)) > length(unique(SampleSheet$Plate))){
plotfactor<-factor(SampleSheet$Time_Point, levels=c(unique(SampleSheet$Time_Point)))
par(mfrow = c(1,1))
...
```{r sex check, echo = FALSE, message = FALSE}
...
##Cell Composition
DNA methylation varies between different cell types, and so the composition of cell types can significantly affect the methylation of samples. Therefore, estimations of cell-type composition is an important variable to consider when analysing this type of data.In order to estimate the blood cell proportions, we use an function estimateCellCounts in the R package Minfi, developed by Hansen et al. We will estimate cell types CD8T, CD4T, Bcells, Granulocytes, Monocytes and NK cells. Samples that have failed until this stage will not be included in calculated cell proportions.

If your tissue is not blood, there is currently no appropriate algorithm to calculate you cell propotions for your tissue of interest. Therefore, this part of the report  will not be run and we will move along to the next part of the report. 

```{r cellcomposition, echo=FALSE, warning= FALSE, message = FALSE}
setwd(params$PathToProjectFolder)
if(SampleSheet$Organ == "Blood"){
  if(file.exists(file= "CellCounts.rdat")){
    print("Loading cell counts")
  load(file = "CellCounts.rdat")
  } else {
#Making cell counts file using referance data set, this takes a while so has already been done
counts <- estimateCellCounts(RGset[,!SamplesFail], compositeCellType ="Blood", cellTypes = c("CD8T","CD4T", "Bcell","Gran", "Mono","NK"), processMethod = "auto")
save(counts, file="CellCounts.rdat")
print(paste("Cell Counts created and saved as, CellCounts.rdat"))
}
...
counts.m <- melt(counts,"colnames")
i <- sapply(counts.m, is.factor) #gets rid of factos in data frame
counts.m[i] <- lapply(counts.m[i], as.character)
colnames(counts.m) <- c("Samples","Cell","Proportion")


boxplot(counts, use.cols=T, main="Boxplot of Estimated Cell Proportions")

ggplot(counts.m, aes(x = Proportion)) +
